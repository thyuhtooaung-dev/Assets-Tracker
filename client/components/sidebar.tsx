    ...
    >
      <header>
        <h1 className={"text-center text-4xl font-semibold"}>
          Assets{" "}
          <span className={"text-[#5F9EA0] dark:text-rose-500"}>Tracker</span>
        </h1>
      </header>
      <section className={"flex flex-col gap-6 w-full"}>
        <Link href={"/"} onClick={handleNavigate} className={navLinkClassName}>
          <Gauge size={18} />
          DashBoard
        </Link>
        <Link
      ...
      <footer
        className={
          "mt-auto flex w-full items-center justify-center gap-4 text-muted-foreground"
        }
      >
        LogOut
        <button
